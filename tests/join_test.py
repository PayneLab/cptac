...
@pytest.mark.parametrize("cancer_combos", get_join_inputs())
def test_join_omics_to_omics(cancer_combos, source_dict):
    cancer_name, source_dict = cancer_combos
    cancer_obj = getattr(cptac, cancer_name.title())
    sources = source_dict['omics']
    for source1, source2 in it.combinations(sources):
        try:
            df = cancer_obj.join_omics_to_omics(
                    df1_name = source1[1],
                    df2_name = source2[1],
            ...
            pytest.fail(f"Unable to perform join on {source1[1]} and {source[2]} for {cancer_name}.\n{e}")
