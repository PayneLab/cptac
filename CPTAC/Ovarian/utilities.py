...
# Next 4 functions are for working with clinical data
    ...
    def get_cols_from_metadata(self, df, cols):
        """Select several columns, given as a list, from the clinical dataframe.
    ...
    def append_metadata_to_omics(self, df, omics_df, df_cols, omics_cols):
        """Selected the specified columns from the clinical dataframe, and append to all or part of the given omics dataframe. Intersection (inner join) of indicies is used.

        Parameters:
        df (pandas DataFrame): Either the clinical  dataframe, from which we'll select our columns to append.
        omics_df (pandas DataFrame): The omics dataframe to append to.
        df_cols (str, or list or array-like of str): Column(s) to select from the clinical  dataframe. str if one column, list or array-like of strings if multiple.
        ...
        Returns:
        pandas DataFrame: The selected columns from the clinical dataframe, appended to the selected columns from the omics dataframe.
        """  
...
# Next 4 functions are for working with mutation data
    def get_mutations_for_gene(self, mutations, gene):
        ...
        gene_mutations = gene_mutations.drop(columns=[gene_col]) # Gene column is same for every sample
    ...
    def append_mutations_to_omics(self, somatic_mutation, omics_df, mutation_genes, omics_genes, show_location):
        ...
        if (omics is not None) and (mutations is not None): # If either selector returned None, then there were gene(s) that didn't match anything, and an error message was printed. We'll return None.
            ...
            merge["Sample_Status"] = np.where(merge.index <= "S116", "Tumor", "Normal") # Add Sample_Status column indicating sample type. Set all samples up to S104 as Tumor, others as normal.
