...
class BcmOv(Cancer):
    ...
    def __init__(self, no_internet, version):
        ...
        data_files = {
            "1.0": [
                "OV-gene_RSEM_tumor_normal_UQ_log2(x+1)_BCM.txt", 
                "gencode.v34.basic.annotation-mapping.txt"
            ]
        }

        # Call the parent class __init__ function
        super().__init__(cancer_type="bcmov", version=version, valid_versions=valid_versions, data_files=data_files, no_internet=no_internet)
        ...
        for file_path in self._data_files_paths: # Loops through files variable
            ...
            if file_name == "OV-gene_RSEM_tumor_normal_UQ_log2(x+1)_BCM.txt":
        ...
        transcript_df = self._data["transcriptomics"]
        gene_key = self._helper_tables["gene_key"]
        transcript = gene_key.join(transcript_df,how = "inner") #keep only gene_ids with gene names
