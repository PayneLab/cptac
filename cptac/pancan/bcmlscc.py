...
class BcmLscc(Dataset):
    ...
    def __init__(self, no_internet, version):
        ...
        data_files = {
            "1.0": [
                "LSCC-circRNA_rsem_tumor_normal_UQ_log2(x+1)_BCM.txt",
                "gencode.v34.basic.annotation-mapping.txt",
                "LSCC-gene_rsem_removed_circRNA_tumor_normal_UQ_log2(x+1)_BCM.txt"
            ]
        }

        # Call the parent class __init__ function
        super().__init__(cancer_type="bcmlscc", version=version, valid_versions=valid_versions, data_files=data_files, no_internet=no_internet)
        ...
        for file_path in self._data_files_paths: # Loops through files variable
            ...
            if file_name == "LSCC-circRNA_rsem_tumor_normal_UQ_log2(x+1)_BCM.txt":
                df = pd.read_csv(file_path, sep="\t")
