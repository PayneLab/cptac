...
class BcmPdac(Dataset):
    ...
    def __init__(self, no_internet, version):
        ...
        data_files = {
            "1.0": [
                "PDAC-gene_rsem_removed_circRNA_tumor_normal_UQ_log2(x+1)_BCM.txt",
                "gencode.v34.basic.annotation-mapping.txt",
                "PDAC-circRNA_rsem_tumor_normal_UQ_log2(x+1)_BCM.txt",
                #"CPTAC_GBM_discovery_CNV_gene_level_log2ratio.tsv.gz"
            ]
        }

        # Call the parent class __init__ function
        super().__init__(cancer_type="bcmpdac", version=version, valid_versions=valid_versions, data_files=data_files, no_internet=no_internet)
        ...
        for file_path in self._data_files_paths: # Loops through files variable
            ...
            if file_name == "PDAC-gene_rsem_removed_circRNA_tumor_normal_UQ_log2(x+1)_BCM.txt":
            ...
            if file_name == "PDAC-circRNA_rsem_tumor_normal_UQ_log2(x+1)_BCM.txt":
                df = pd.read_csv(file_path, sep="\t")
