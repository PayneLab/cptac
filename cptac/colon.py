    ...
    def __init__(self):
        ...
        for file in files: # Loops through files variable
            ...
            if df_name == "clinical" or df_name.split("_")[0] == "proteomics" or df_name.split("_")[0] == "transcriptomics":
                ...
                df = df.transpose()
                df.name = df_name
            elif df_name.split("_")[0] == "phosphoproteomics": #column names are SLC12A8_T485_A0AV02:T485, should this be changed? Yes!
                df = pd.read_csv(file, sep="\t",index_col=0)
                df = df.transpose()
                df.name = df_name
                ...
                df = df.transpose()
                df.name = df_name
            elif df_name == "mutation_binary":
                df = pd.read_csv(file, sep="\t",index_col=0)
                df = df.transpose()
                df.name = "somatic_" + df_name
            elif df_name == "mutation":
